...
class CreateProject(TemplateCommand):
    ...
    """
    help = "Creates the directory structure for a new Kitso CMS project."
    ...
    def handle(self, **options):
        ...
        import kitsocms
        kitso_path = os.path.dirname(kitsocms.__file__)
        if not options['template']:
            options['template'] = 'basic'
        template_path = os.path.join(
            os.path.join(kitso_path, 'project_template'),
            options['template']
        )
        # Check if provided template is built-in to kitsocms,
        ...
        # Set options
        message = "Creating a Kitso CMS project called %(project_name)s"

"""
Base and abstract pages used in Kitso CMS.
"""
...
from kitsocms import schema, utils
from kitsocms.blocks import (
...
from kitsocms.fields import ColorField
from kitsocms.forms import KitsoFormBuilder, KitsoSubmissionsListView
from kitsocms.models.snippet_models import ClassifierTerm
from kitsocms.models.wagtailsettings_models import GeneralSettings, LayoutSettings, SeoSettings, GoogleApiSettings  # noqa
from kitsocms.wagtail_flexible_forms.blocks import FormFieldBlock, FormStepBlock
from kitsocms.wagtail_flexible_forms.models import (
    Step,
    Steps,
    StreamFormMixin,
...
from kitsocms.settings import cr_settings
from kitsocms.widgets import ClassifierSelectWidget


logger = logging.getLogger('kitsocms')
...
def get_page_models():
    return KITSO_PAGE_MODELS


class KitsoPageMeta(PageBase):
    def __init__(cls, name, bases, dct):
        ...
        if 'search_template' not in dct:
            cls.search_template = 'kitsocms/pages/search_result.html'
        if not cls._meta.abstract:
            KITSO_PAGE_MODELS.append(cls)


class KitsoTag(TaggedItemBase):
    class Meta:
        verbose_name = _('Kitso Tag')
    content_object = ParentalKey('kitsocms.KitsoPage', related_name='tagged_items')


class KitsoPage(WagtailCacheMixin, Page, metaclass=KitsoPageMeta):
    """
    General use page with caching, templating, and SEO functionality.
    All pages should inherit from this.
    """
    class Meta:
        verbose_name = _('Kitso Page')
    ...
    index_query_pagemodel = 'kitsocms.KitsoPage'
    ...
    def get_template(self, request, *args, **kwargs):
        ...
        return super(KitsoPage, self).get_template(request, args, kwargs)
                ...
                if len(terms) > 0:
                    ...
                    if len(selected_terms) > 0:
                        try:
                            ...
                            logger.warning(
                                "Tried to filter by ClassifierTerm, but <%s.%s ('%s')>.get_index_children() did not return a queryset or is not a queryset of KitsoPage models.",  # noqa
...
class KitsoArticlePage(KitsoWebPage):
    """
    Article, suitable for news or blog content.
    """
    class Meta:
        verbose_name = _('Kitso Article')
        abstract = True
    ...
    content_panels = KitsoWebPage.content_panels + [
        FieldPanel('caption'),
        MultiFieldPanel(
            [
                FieldPanel('author'),
...
class KitsoArticleIndexPage(KitsoWebPage):
    """
    Shows a list of article sub-pages.
    """
    class Meta:
        verbose_name = _('Kitso Article Index Page')
        abstract = True
    ...
    index_order_by_choices = (('-date_display', 'Display publish date, newest first'),) + \
        KitsoWebPage.index_order_by_choices
    ...
    layout_panels = KitsoWebPage.layout_panels + [
        MultiFieldPanel(
            [
...
class KitsoEventPage(KitsoWebPage, BaseEvent):
    class Meta:
        verbose_name = _('Kitso Event')
        abstract = True
...
class KitsoEventIndexPage(KitsoWebPage):
    """
    Shows a list of event sub-pages.
    """
    class Meta:
        verbose_name = _('Kitso Event Index Page')
        abstract = True

    template = 'kitsocms/pages/event_index_page.html'
...
class KitsoEventOccurrence(Orderable, BaseOccurrence):
    class Meta:
        verbose_name = _('Kitso Event Occurrence')
        abstract = True


class KitsoFormMixin(models.Model):
    class Meta:
        abstract = True

    submissions_list_view_class = KitsoSubmissionsListView
...
class KitsoFormPage(KitsoFormMixin, KitsoWebPage):
    """
    This is basically a clone of wagtail.contrib.forms.models.AbstractForm
    with changes in functionality and extending KitsoWebPage vs wagtailcore.Page.
    """
    class Meta:
        verbose_name = _('Kitso Form Page')
        abstract = True

    template = 'kitsocms/pages/form_page.html'
    landing_page_template = 'kitsocms/pages/form_page_landing.html'

    base_form_class = WagtailAdminFormPageForm

    form_builder = KitsoFormBuilder

    body_content_panels = [
        InlinePanel('form_fields', label="Form fields"),
    ] + \
        KitsoWebPage.body_content_panels + \
        KitsoFormMixin.body_content_panels + [
            FormSubmissionsPanel(),
            InlinePanel('confirmation_emails', label=_('Confirmation Emails'))
    ]

    settings_panels = KitsoPage.settings_panels + KitsoFormMixin.settings_panels
...
class KitsoSubmissionRevision(SubmissionRevision, models.Model):
    pass


class KitsoSessionFormSubmission(SessionFormSubmission):
    ...
    def create_normal_submission(self, delete_self=True):
        ...
        if delete_self:
            KitsoSubmissionRevision.objects.filter(submission_id=self.id).delete()
            self.delete()
...
def create_submission_changed_revision(sender, **kwargs):
    ...
    KitsoSubmissionRevision.create_from_submission(
        submission, (KitsoSubmissionRevision.CREATED if created else KitsoSubmissionRevision.CHANGED))  # noqa


@receiver(post_delete)
def create_submission_deleted_revision(sender, **kwargs):
    if not issubclass(sender, KitsoSessionFormSubmission):
        return
    submission = kwargs['instance']
    KitsoSubmissionRevision.create_from_submission(submission, SubmissionRevision.DELETED)  # noqa


class KitsoStep(Step):
...
class KitsoSteps(Steps):
    ...
    def __init__(self, page, request=None):
        ...
        has_steps = any(isinstance(struct_child.block, FormStepBlock)
                        for struct_child in self.form_fields)
        if has_steps:
            steps = [KitsoStep(self, i, form_field)
                     for i, form_field in enumerate(self.form_fields)]
        else:
            steps = [KitsoStep(self, 0, self.form_fields)]
        super(Steps, self).__init__(steps)


class KitsoStreamFormMixin(StreamFormMixin):
    class Meta:
        abstract = True

    def get_steps(self, request=None):
        if not hasattr(self, 'steps'):
            steps = KitsoSteps(self, request=request)
            if request is None:
                return steps
            self.steps = steps
        return self.steps
    ...
    def get_session_submission_class():
        return KitsoSessionFormSubmission
...
class KitsoStreamFormPage(KitsoFormMixin, KitsoStreamFormMixin, KitsoWebPage):
    class Meta:
        verbose_name = _('Kitso Advanced Form Page')
        abstract = True

    template = 'kitsocms/pages/stream_form_page.html'
    landing_page_template = 'kitsocms/pages/form_page_landing.html'
    ...
    def process_form_get(self, form, request):
        return KitsoWebPage.serve(self, request)
...
class KitsoLocationPage(KitsoWebPage):
    """
    Location, suitable for store locations or help centers.
    """
    class Meta:
        verbose_name = _('Kitso Location')
        abstract = True

    template = 'kitsocms/pages/location_page.html'
    ...
    settings_panels = KitsoWebPage.settings_panels + [
        MultiFieldPanel(
            [
    ...
    def render_pin_description(self):
        return render_to_string(
            'kitsocms/includes/map_pin_description.html',
            {
                'page': self
            }
        )
    ...
    def render_list_description(self):
        return render_to_string(
            'kitsocms/includes/map_list_description.html',
            {
                'page': self
            }
        )
    ...
    def save(self, *args, **kwargs):
        ...
        return super(KitsoLocationPage, self).save(*args, **kwargs)
...
class KitsoLocationIndexPage(KitsoWebPage):
    """
    Shows a map view of the children KitsoLocationPage.
    """
    class Meta:
        verbose_name = _('Kitso Location Index Page')
        abstract = True

    template = 'kitsocms/pages/location_index_page.html'
